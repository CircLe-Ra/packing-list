<?php

use function Livewire\Volt\{layout, title, state, computed, on, usesPagination, mount, updated, usesFileUploads};
use App\Models\ShipmentDetail;
use App\Models\Shipment;
use App\Models\Container;
use App\Models\ShipmentItem;
use App\Models\Distribution;
use Masmerise\Toaster\Toaster;
use App\Models\Driver;
use Illuminate\Support\Facades\DB;
use App\Models\ItemDamage;
use App\Models\Consumer;
use App\Models\Delivery;

layout('layouts.app');
title(__('Verify Truck'));

state(['shipment_id' => fn($shipment) => $shipment])->locked();
state(['showing' => 6, 'search' => null])->url();
state(['idData', 'shipment', 'consumer_id', 'address', 'phone', 'departure_date', 'driver_id']);
state(['loading' => false, 'drivers' => [], 'distributions' => [], 'consumers' => [], 'openItem' => false]);

usesPagination();

mount(function () {
    if (!Shipment::where('id', $this->shipment_id)->exists()) {
        abort(404);
    }
    $this->shipment = Shipment::with('shipment_details.shipment_items.distributions.driver')->find($this->shipment_id);
    $this->drivers = DB::table('shipments')
        ->join('shipment_details', 'shipments.id', '=', 'shipment_details.shipment_id')
        ->join('shipment_items', 'shipment_details.id', '=', 'shipment_items.shipment_detail_id')
        ->join('distributions', 'shipment_items.id', '=', 'distributions.shipment_item_id')
        ->join('drivers', 'distributions.driver_id', '=', 'drivers.id')
        ->leftJoin('deliveries', function ($join) {
            $join->on('deliveries.driver_id', '=', 'drivers.id')
                ->on('deliveries.shipment_id', '=', 'shipments.id');
        })
        ->where('shipments.id', $this->shipment_id)
        ->select(
            'drivers.*',
            'deliveries.status'
        )
        ->distinct()
        ->get();


    $this->consumers = Consumer::all();
});

on(
    [
        'refresh' => function () {
            $this->drivers = DB::table('shipments')
                ->join('shipment_details', 'shipments.id', '=', 'shipment_details.shipment_id')
                ->join('shipment_items', 'shipment_details.id', '=', 'shipment_items.shipment_detail_id')
                ->join('distributions', 'shipment_items.id', '=', 'distributions.shipment_item_id')
                ->join('drivers', 'distributions.driver_id', '=', 'drivers.id')
                ->leftJoin('deliveries', function ($join) {
                    $join->on('deliveries.driver_id', '=', 'drivers.id')
                        ->on('deliveries.shipment_id', '=', 'shipments.id');
                })
                ->where('shipments.id', $this->shipment_id)
                ->select(
                    'drivers.*',
                    'deliveries.status'
                )
                ->distinct()
                ->get();
        },
        'close-modal-x' => function() {
            $this->idData = null;
        },
        'show-item-details' => function($data) {
            $this->loading = true;
            $this->idData = $data;
        },
        'close' => function() {
            $this->dispatch(['close-modal', 'modal_item_details']);
            $this->idData = null;
        },
        'set-data' => fn ($data) => $this->driver_id = $data,
    ]
);

updated([
    'consumer_id' => function ($value) {
        $this->openItem = (bool) $value;
        $consumer = Consumer::find($value);
        $this->phone = $consumer->phone;
        $this->address = $consumer->address;
    },
]);

$getDistributionItems = function ($data) {
    $distributions = Distribution::where('driver_id', $data)->get();
    $this->loading = false;
    return $distributions;
};

$prefer = function () {
    $validate = $this->validate([
        'consumer_id' => 'required|exists:consumers,id',
        'departure_date' => 'required',
    ]);

    $validate['driver_id'] = $this->driver_id;
    $validate['shipment_id'] = $this->shipment_id;
    $validate['status'] = 'prefer';

    try {
        Delivery::create($validate);
        $this->dispatch('close-modal', 'modal_verify');
        $this->dispatch('refresh');
        $this->reset(['driver_id', 'consumer_id', 'address', 'phone', 'departure_date']);
        Toaster::success(__('Verification truck has been preferred'));
    } catch (Exception $e) {
        $this->reset(['driver_id', 'consumer_id', 'address', 'phone', 'departure_date']);
        $this->dispatch('close-modal', 'modal_verify');
        Toaster::error(__('Verification truck could not be preferred'));
    }

}

?>
<div>
        <div class="flex justify-between">
        <x-breadcrumb :crumbs="[
                    [
                        'text' => __('Dashboard'),
                        'href' => '/dashboard',
                    ],
                    [
                        'text' => __('Submissions')
                    ],
                    [
                        'text' => __('Delivery Order'),
                        'href' => route('submissions.delivery-order'),
                    ],
                    [
                        'text' => $this->shipment->loader_ship,
                    ],
                    [
                        'text' => __('Verify Truck'),
                    ]
                ]" class="items-start"
        />
    </div>

        <x-form.modal id="modal_verify" class="-mt-2 w-9/12 max-w-xl" :title="__('Verify Truck')">
            <x-select-input name="consumer_id" id="consumer_id" display_name="name" wire:model.live="consumer_id" labelClass="-mt-4 mb-3" title="{{ __('Consumer') }}" :data="$this->consumers" getData="server"/>
            <div x-data="{ open : $wire.openItem }">
                <div x-cloak
                     x-show="$wire.openItem"
                     x-transition:enter="transition ease-out duration-300"
                     x-transition:enter-start="transform opacity-0"
                     x-transition:enter-end="transform opacity-100"
                     x-transition:leave="transition ease-in duration-200"
                     x-transition:leave.start="transform opacity-100"
                     x-transition:leave-end="transform opacity-0">
                    <div>
                        <x-text-input-4 type="text" name="phone" wire:model="phone" labelClass="mb-3 " title="{{ __('Phone Number') }}" :disabled="true" />
                    </div>
                    <div>
                        <x-text-input-4 type="text" name="address" wire:model="address" labelClass="mb-3 " title="{{ __('Address') }}" :disabled="true" />
                    </div>
                    <div>
                        <x-text-input-4 type="date" name="departure_date" wire:model="departure_date" labelClass="mb-3 " title="{{ __('Departure Date') }}" />
                    </div>
                    <div class="flex justify-end space-x-3">
                        <x-button-info class="text-white" wire:click="prefer">{{ __('Prefer') }}</x-button-info>
                    </div>
                </div>
            </div>
        </x-form.modal>

        <x-form.modal id="modal_item_details" class="-mt-2 w-9/12 max-w-xl" :title="__('Item Details')">
            <div>
                @if(!$this->loading)
                    <div>
                        <div class="flex justify-center items-center">
                            <span class="loading loading-dots loading-lg"></span>
                        </div>
                    </div>
                @else
                    <table class="min-w-full divide-y divide-base-100">
                        <thead class="bg-neutral ">
                        <tr>
                            <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-neutral-content uppercase tracking-wider">
                                {{ __('Item Name') }}
                            </th>
                            <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-neutral-content uppercase tracking-wider">
                                {{ __('Container Info') }}
                            </th>
                            <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-neutral-content uppercase tracking-wider">
                                {{ __('Quantity') }}
                            </th>
                        </tr>
                        </thead>
                        <tbody class=" divide-y divide-base-100">
                        @foreach($this->getDistributionItems($this->idData) as $key => $distribution)
                            <tr class="{{ $key % 2 == 0 ? 'bg-base-200' : 'bg-base-300' }}">
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-base-content">
                                    {{ $distribution->shipment_item->item_name }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-base-content">
                                    {{ $distribution->shipment_item->shipment_detail->container->number_container }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-base-content">
                                    {{ $distribution->quantity }}
                                </td>
                            </tr>
                        @endforeach
                        </tbody>
                    </table>
                @endif
                <div class="flex justify-end mt-4">
                    <label for="modal_item_details" class="text-white btn btn-error" @click="$dispatch('close-modal-x')">{{ __('Close') }}</label>
                </div>
            </div>
        </x-form.modal>

    <div>
        <h2 class="card-title">{{ __('Data Truck Delivery') }}</h2>
        <div class="flex flex-wrap items-center justify-between py-4 space-y-4 flex-column sm:flex-row sm:space-y-0">
            <x-form.filter class="w-24 text-xs select-sm" wire:model.live="showing" :select="['5', '10', '20', '50', '100']"  />
            <x-form.search wire:model.live="search" class="w-32" />
        </div>

        @if($this->drivers && $this->drivers->isNotEmpty())
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2 mb-10">
                @foreach($this->drivers as $key => $driver)
                        <x-card class="w-full max-w-1/5 md:w-full bg-base-300 border border-base-200 rounded-lg shadow sm:p-2 relative">
                            <div class="absolute top-2 right-2">
                                @if($driver->status == 'verified')
                                    <div class="tooltip" data-tip="{{ __('Verified') }}">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24"><path fill="#4d88ff" d="m8.6 22.5l-1.9-3.2l-3.6-.8l.35-3.7L1 12l2.45-2.8l-.35-3.7l3.6-.8l1.9-3.2L12 2.95l3.4-1.45l1.9-3.2l3.6.8l-.35 3.7L23 12l-2.45 2.8l.35 3.7l-3.6.8l-1.9 3.2l-3.4-1.45zm2.35-6.95L16.6 9.9l-1.4-1.45l-4.25 4.25l-2.15-2.1L7.4 12z"/></svg>
                                    </div>
                                @elseif($driver->status == 'prefer')
                                    <div class="tooltip" data-tip="{{ __('Preferred') }}">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 16 16"><path fill="#ffe252" fill-rule="evenodd" d="M12 6.5a1.5 1.5 0 1 0 0-3a1.5 1.5 0 0 0 0 3M12 8a3 3 0 1 0-2.905-3.75H1.75a.75.75 0 0 0 0 1.5h7.345A3 3 0 0 0 12 8m-6.5 3a1.5 1.5 0 1 1-3 0a1.5 1.5 0 0 1 3 0m1.405.75A3.001 3.001 0 0 1 1 11a3 3 0 0 1 5.905-.75h7.345a.75.75 0 0 1 0 1.5z" clip-rule="evenodd"/></svg>
                                    </div>
                                @else
                                    <div class="tooltip" data-tip="{{ __('Pending') }}">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24"><path fill="#858585" fill-rule="evenodd" d="M8 1a1 1 0 0 0-.716.302l-6 6.156A1 1 0 0 0 1 8.156V16a1 1 0 0 0 .293.707l6 6A1 1 0 0 0 8 23h8a1 1 0 0 0 .707-.293l6-6A1 1 0 0 0 23 16V8.156a1 1 0 0 0-.284-.698l-6-6.156A1 1 0 0 0 16 1zm0 10a1 1 0 1 0 0 2h8a1 1 0 1 0 0-2z" clip-rule="evenodd"/></svg>
                                    </div>
                                @endif
                            </div>
                            <div class="flex flex-col text-center mb-4">
                                <h5 class="text-xl font-bold text-base-content leading-5">
                                    <span class="block mb-2">{{ __('Truck') }}</span>
                                    <span>{{ $driver->vehicle_number }}</span>
                                </h5>
                            </div>
                            <ul role="list" class="divide-y divide-neutral-600">
                                <li class="py-3 sm:py-4 flex flex-row items-center justify-between">
                                    <div>
                                        <h3 class="text-lg font-bold text-base-content ">
                                            {{ __('Driver Name') }}
                                        </h3>
                                    </div>
                                    <div>
                                        <p class="text-sm text-base-content">
                                            {{ $driver->name }}
                                        </p>
                                    </div>
                                </li>
                                <li class="py-3 sm:py-4 flex flex-row items-center justify-between">
                                    <div>
                                        <h3 class="text-lg font-bold text-base-content ">
                                            {{ __('Driver Phone') }}
                                        </h3>
                                    </div>
                                    <div>
                                        <p class="text-sm text-base-content">
                                            {{ $driver->phone }}
                                        </p>
                                    </div>
                                </li>
                                <li class="py-3 sm:py-4 flex flex-row items-center justify-between">
                                    <div>
                                        <h3 class="text-lg font-bold text-base-content">
                                            <label for="modal_item_details" class="inline-flex items-center hover:underline cursor-pointer" wire:click="$dispatch('show-item-details', { data: {{ $driver->id }} })">
                                                {{ __('Item In Truck') }}
                                                <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 30 30" class="pt-1">
                                                    <rect width="30" height="30" fill="none" />
                                                    <path fill="currentColor" d="M10.05 17.55c0 .3.09.55.26.73c.2.19.46.28.79.28c.3 0 .55-.09.73-.28l6.04-6.05v1.95q0 .45.3.75t.75.3c.29 0 .54-.1.74-.31s.3-.45.3-.75V9.7q0-.45-.3-.75c-.3-.3-.45-.3-.74-.3h-4.5c-.29 0-.54.1-.73.3s-.29.44-.29.75c0 .29.1.54.29.73s.44.29.73.29h1.95l-6.06 6.06c-.17.21-.26.47-.26.77" />
                                                </svg>
                                            </label>
                                        </h3>
                                    </div>
                                    <div>
                                        <p class="text-sm text-base-content">
                                            {{ __('Details of items') }}
                                        </p>
                                    </div>
                                </li>
                                @if($driver->status == '')
                                <li class="pt-3 sm:pt-4 flex flex-row items-center justify-end gap-2">
                                    <label for="modal_verify" class="btn btn-sm btn-success text-white" @click="$dispatch('set-data', { data: '{{ $driver->id }}' })">{{ __('Prefer') }}</label>
                                </li>
                                @endif
                            </ul>
                        </x-card>
                @endforeach
            </div>
        @else
            <div class="m-auto text-center">
                {{ __('No Data') }}
            </div>
        @endif
    </div>
</div>
