    ...
    <x-form.modal id="modal_item_damaged" class="-mt-2 w-9/12 max-w-xl" :title="__('Damaged Items')">
        ...
        <div class="{{ $this->loading ? 'space-y-4' : 'space-y-4 hidden' }}">
            @foreach ($this->loadDamagedItems($this->idData) as $item)
                @if (!empty($item->image))
                    <div class="flex justify-center items-center">
                        <img src="{{ asset('storage/' . $item->image) }}" alt="Damaged Item" class="max-w-full h-auto mb-4">
                    </div>
                @endif
            @endforeach
        </div>
                ...
                @foreach($this->shipment_items as $key => $shipment_item)
                        ...
                        <x-card class="w-full max-w-1/5 md:w-full bg-base-300 border border-base-200 rounded-lg shadow sm:p-2 relative">
                            ...
                            <ul role="list" class="divide-y divide-gray-200 dark:divide-gray-700">
                                ...
                                <li class="py-3 sm:py-4 flex flex-row items-center justify-between">
                                    <div>
                                        <label for="{{ $shipment_item->item_damaged <= 0 ? '' : 'modal_item_damaged' }}" class="{{ $shipment_item->item_damaged <= 0 ? 'tooltip cursor-not-allowed' : '' }}" data-tip="{{ __('No Damaged Items') }}" wire:click="$dispatch('show-item-damaged', { data: {{ $shipment_item->id }} })">
                                        <h3 class="text-lg font-bold text-base-content inline-flex items-center hover:underline cursor-pointer">
                                            {{ __('Damaged Items') }}
