...
on([
    ...
    'rename-title-and-value' => function ($data, $name) {
        ...
        $getData = ShipmentItem::select(
            ...
            ->leftJoin(DB::raw('(SELECT shipment_item_id, SUM(quantity) AS quantity FROM distributions GROUP BY shipment_item_id) AS distributions'), 'distributions.shipment_item_id', '=', 'shipment_items.id')
    ...
    'show-item-damaged' => function ($data) {
        $this->idData = $data;
        $this->loading = true;
    },
]);
...
$loadDamagedItems = function ($id) {
    $damaged = ItemDamage::where('shipment_item_id', $id)->get();
    $this->loading = false;
    return $damaged;
};
...
$print = function ($id) {
    ...
    $pdf = Pdf::loadView('livewire.report.damaged-item', ['data' => $data]);
...
<div>
    ...
    <x-form.modal id="modal_item_damaged" class="-mt-2 w-9/12 max-w-xl" :title="__('Damaged Items')">
        <div class="{{ $this->loading ? 'hidden' : '' }}">
            <div class="flex justify-center items-center">
                <span class="loading loading-dots loading-lg"></span>
            </div>
        </div>
        <div class="{{ $this->loading ? 'space-y-4' : 'space-y-4 hidden' }}">
            @foreach ($this->loadDamagedItems($this->idData) as $item)
                @if (!empty($item->image))
                    <div class="flex justify-center items-center">
                        <img src="{{ asset('storage/' . $item->image) }}" alt="Damaged Item"
    ...
    <x-form.modal id="modal_add_damaged_item" class="-mt-2 w-9/12 max-w-xl" :title="$this->modalTitle ? __('Damaged Items') . ' ' . $this->modalTitle : __('Damaged Items')">
        <div>
            <x-text-input-4 type="number" name="damagedQuantity" wire:model="damagedQuantity" labelClass="mb-3 -mt-4"
                title="{{ __('Damaged Item Quantity') }}" />
            <div>
    ...
    <x-form.modal id="modal_add_checklist" class="-mt-2 w-9/12 max-w-xl" :title="$this->modalTitle ? __('Checklist Data') . ' ' . $this->modalTitle : __('Checklist Data')">
        <x-select-input name="driver_id" id="driver_id" display_name="vehicle_number,name" wire:model.live="driver_id"
            labelClass="-mt-4 mb-3" title="{{ __('Driver') }}" :data="$this->drivers" getData="server" />
        <div x-data="{ open: $wire.openAddItem }">
            <div x-cloak x-show="$wire.openAddItem" x-transition:enter="transition ease-out duration-300"
                ...
                <div class="flex justify-between space-x-3">
                    <div class="w-1/2 flex space-x-3 justify-between">
                        <div>
                            <x-text-input-4 type="number" name="itemDamaged" wire:model="itemDamaged"
                                labelClass="mb-3 " title="{{ __('Damaged Items') }}" :disabled="true" />
                        </div>
                        <div>
                            <x-text-input-4 type="number" name="qtyInfo" wire:model="qtyInfo" labelClass="mb-3 "
                                title="{{ __('Unchecklist') }}" :disabled="true" />
                        </div>
                    </div>
                    <div class="w-1/2">
                        <x-text-input-4 type="number" name="quantity" wire:model="quantity" labelClass="mb-3 "
                            title="{{ __('Item In Truck') }}" />
                    </div>
                </div>
                        ...
                        <ul role="list" class="divide-y divide-gray-200 dark:divide-gray-700">
                                    ...
                                    <p class="text-sm text-base-content">
                                        @php
                                            $quantity = ShipmentItem::select(
                                                ...
                                                ->leftJoin(
                                                    DB::raw(
                                                        '(SELECT shipment_item_id, SUM(quantity) AS quantity FROM distributions GROUP BY shipment_item_id) AS distributions',
                                                    ),
                                                    'distributions.shipment_item_id',
                                                    '=',
                                                    'shipment_items.id',
                                                )
                                                ->where('shipment_items.id', $shipment_item->id)
                                                ->first();
                                        @endphp
                                        {{ $quantity->leftover . ' ' . __('Cardboard') }}
                                    </p>
                                </div>
                            </li>
                            <li class="py-3 sm:py-4 flex flex-row items-center justify-between">
                                <div>
                                    <label for="{{ $shipment_item->item_damaged <= 0 ? '' : 'modal_item_damaged' }}"
                                        class="{{ $shipment_item->item_damaged <= 0 ? 'tooltip cursor-not-allowed' : '' }}"
                                        data-tip="{{ __('No Damaged Items') }}"
                                        wire:click="$dispatch('show-item-damaged', { data: {{ $shipment_item->id }} })">
                                        <h3
                                            class="text-lg font-bold text-base-content inline-flex items-center hover:underline cursor-pointer">
                                            {{ __('Damaged Items') }}
                            ...
                            <li class="py-3 sm:py-4 items-center">
                                <div x-data="{ open: false }">
                                    <h3 @click="open = !open"
                                        class="flex flex-row items-center justify-between text-lg text-base-content font-bold
                                        {{ $shipment_item->distributions->count() > 0 ? 'cursor-pointer' : 'cursor-not-allowed tooltip tooltip-top' }} "
                                        data-tip="{{ __('No Truck List') }}">{{ __('Truck List') }}
                                        ({{ $shipment_item->distributions->count() }})
                                        ...
                                        <ul role="list" class="divide-y divide-gray-200 dark:divide-gray-700">
                                            @foreach ($shipment_item->distributions as $key => $distribution)
                                                <li class="py-3 sm:py-4 flex flex-row items-center justify-between">
                                                    <div>
                                                        <p class="text-sm text-base-content">
                                                            {{ $distribution->driver->vehicle_number }} :
                                                    ...
                                                    <div class="flex items-center space-x-2">
                                                        ...
                                                        @if ($distribution->driver->status == 'free')
                                                            <label for="modal_confirm_delete"
                                                                class="btn btn-xs btn-error text-white"
                                                                @click="$dispatch('confirm-delete-distribution', { data: '{{ $distribution->id }}'})">
                                                                {{ __('Delete') }}
                                                            </label>
                                                        @endif
                                                    </div>
                            ...
                            <li class="py-3 sm:py-4 flex flex-col items-center justify-center gap-2">
                                <div>
                                    <label for="modal_add_checklist" class="btn btn-sm btn-success text-white"
                                        @click="$dispatch('rename-title-and-value', { data: '{{ $shipment_item->id }}', name: '{{ $shipment_item->item_name }}' })">{{ __('Checklist Items') }}</label>
                                    <label for="modal_add_damaged_item" class="btn btn-sm btn-error text-white"
                                        @click="$dispatch('open-damaged-modal', { data: '{{ $shipment_item->id }}', name: '{{ $shipment_item->item_name }}' })">{{ __('Damaged Items') }}</label>
                                </div>
                                @if ($shipment_item->item_damaged > 0)
                                    <a href="{{ route('shipment.item-damage.print', $this->shipment_id) }}"
                                        target="_blank"
                                        class="text-white btn btn-error mt-1 btn-sm">{{ __('Print Damaged Items Report') }}</a>
                                @endif
                            </li>
                        </ul>
                    </x-card>
