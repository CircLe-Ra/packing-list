...
state(['idData', 'delivery_id', 'km_image_start', 'vehicle_image_start', 'km_image_end', 'vehicle_image_end']);
...
mount(function () {
    ...
    $this->deliveries = Delivery::whereIn('status', ['delivered', 'verified', 'success'])->where('driver_id', auth()->user()->id)->get();
});

on(
    [
        'refresh' => fn() => $this->deliveries = Delivery::whereIn('status', ['delivered', 'verified', 'success'])->where('driver_id', auth()->user()->id)->get(),
...
$saveImageStart = function () {
    $validate = $this->validate([
        'km_image_start' => 'required|image',
        'vehicle_image_start' => 'required|image',
    ]);
    try {
        Delivery::where('id', $this->delivery_id)->update(['status' => 'delivered']);
        $km_image_start = $this->km_image_start->store('delivery_images', 'public');
        $vehicle_image_start = $this->vehicle_image_start->store('delivery_images', 'public');
        \App\Models\DeliveryImage::create([
            'delivery_id' => $this->delivery_id,
            'km_image_start' => $km_image_start,
            'vehicle_image_start' => $vehicle_image_start,
        ]);
        $this->reset(['km_image_start', 'vehicle_image_start', 'delivery_id']);
        $this->dispatch('refresh');
        $this->dispatch('close-modal', 'modal_upload_km');
        ...
        $this->dispatch('refresh');
        $this->dispatch('close-modal', 'modal_upload_km');
        Toaster::error('Images could not be saved');
    }
};

$saveImageEnd = function () {
    $validate = $this->validate([
        'km_image_end' => 'required|image',
        'vehicle_image_end' => 'required|image',
    ]);
    try {
        ...
        $km_image_end = $this->km_image_end->store('delivery_images', 'public');
        $vehicle_image_end = $this->vehicle_image_end->store('delivery_images', 'public');
        \App\Models\DeliveryImage::where('delivery_id', $this->delivery_id)->update([
            'km_image_end' => $km_image_end,
            'vehicle_image_end' =>  $vehicle_image_end,
        ]);
...
<div>
    ...
    <x-form.modal id="modal_upload_km" class="-mt-2 w-9/12 max-w-xl" :title="__('Upload Report Vehicle Start')">
        <div>
            <div>
                <label class="block mb-2 text-sm font-medium">{{ __('Upload KM Images') }}</label>
                <x-filepond wire:model="km_image_start" />
                @error('km_image_start')<span class="text-sm font-medium text-red-600">{{ $message }}</span>@enderror
            </div>
            <div>
                <label class="block mb-2 text-sm font-medium">{{ __('Upload Vehicle Images') }}</label>
                <x-filepond wire:model="vehicle_image_start" />
                @error('vehicle_image_start')<span class="text-sm font-medium text-red-600">{{ $message }}</span>@enderror
            </div>
            <div class="flex justify-end mt-5">
                <button class="text-white btn btn-info" wire:click="saveImageStart">{{ __('Save') }}</button>
            </div>
        </div>
    </x-form.modal>

    <x-form.modal id="modal_upload_km_end" class="-mt-2 w-9/12 max-w-xl" :title="__('Upload Report Vehicle End')">
        <div>
            <div>
                <label class="block mb-2 text-sm font-medium">{{ __('Upload KM Images') }}</label>
                <x-filepond wire:model="km_image_end" />
                @error('km_image_end')<span class="text-sm font-medium text-red-600">{{ $message }}</span>@enderror
            </div>
            <div>
                <label class="block mb-2 text-sm font-medium">{{ __('Upload Vehicle Images') }}</label>
                <x-filepond wire:model="vehicle_image_end" />
                @error('vehicle_image_end')<span class="text-sm font-medium text-red-600">{{ $message }}</span>@enderror
            </div>
            <div class="flex justify-end mt-5">
    ...
    <div>
        <h2 class="card-title">{{ __('Data Travel Documents') }}</h2>
        <div class="flex flex-wrap items-center justify-between py-4 space-y-4 flex-column sm:flex-row sm:space-y-0">
            <x-form.filter class="w-24 text-xs select-sm" wire:model.live="showing"
                    ...
                    <x-card
                        ...
                        <ul role="list" class="divide-y divide-neutral-600">
                            ...
                            <li class="pt-3 sm:pt-4 flex flex-row items-center justify-end gap-2">
                                @if($delivery->status == 'verified')
                                    <label for="modal_upload_km" class="btn btn-sm btn-success text-white" @click="$dispatch('set-data', { data: '{{ $delivery->id }}' })">{{ __('Upload KM and Vehicle Start') }}</label>
                                @elseif($delivery->status == 'delivered')
                                    <label for="modal_upload_km_end" class="btn btn-sm btn-info text-white" @click="$dispatch('set-data', { data: '{{ $delivery->id }}' })">{{ __('Upload KM and Vehicle End') }}</label>
                                @elseif($delivery->status == 'success')
                                    <a target="_blank" href="{{ route('delivery.print', $delivery->id) }}" class="btn btn-sm btn-info text-white">
                                        {{ __('Print') }}
                                    </a>
                                @endif
                            </li>
